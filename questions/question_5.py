...
print(f"[MongoDB] Nombre d'utilisateurs différents qui ont twetté un tweet contenant le hashtag 'valls': {user_count}")
db.close()

neo = Neo4J()
query = "MATCH (t:Tweet)-[:HAS_HASHTAG]->(h:Hashtag) WHERE h.name = 'valls' RETURN COUNT(DISTINCT t.idUser)"
user_count = neo.query(query)
print(f"[Neo4J] Nombre d'utilisateurs différents qui ont twetté un tweet contenant le hashtag 'valls': {user_count}")
neo.close()
